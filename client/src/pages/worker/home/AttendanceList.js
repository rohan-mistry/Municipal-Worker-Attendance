...
function AttendanceList() {
  ...
  const getList = async(pageNo,limit) => {
    ...
    const result = await fetch(`${url}/attendance/${_id}?page=${pageNo}&limit=${limit}&search=${text.trim()}&start=${start?Date.parse(start):0}&end=${end?Date.parse(end):Date.now()}`,requestOptions);
